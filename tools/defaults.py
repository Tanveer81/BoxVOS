...
from tools.scheduler_util import WarmupReduceLROnPlateau
import math
...
class LRScheduler(HookBase):
    ...
    def after_step(self):
        ...
        if self.trainer.cfg.SOLVER.LR_SCHEDULER_NAME == 'WarmupReduceLRonPlatue':
            if hasattr(self.trainer, '_last_eval_results'):
                if 'segm' in self.trainer._last_eval_results:
                    metric = self.trainer._last_eval_results['segm']['AP']
                else:
                    metric = 0
            else:
                metric = 0
...
class DefaultTrainer(SimpleTrainer):
    ...
    def __init__(self, cfg):
        ...
        if comm.get_world_size() > 1:
            model = DistributedDataParallel(
                ...
                find_unused_parameters=cfg['find_unused_parameters']=="True"
            )
    ...
    def build_lr_scheduler(cls, cfg, optimizer):
        ...
        """
        if cfg.SOLVER.LR_SCHEDULER_NAME == 'WarmupReduceLRonPlatue':
            return WarmupReduceLROnPlateau(optimizer,
                                            warmup_factor=cfg.SOLVER.WARMUP_FACTOR,
                                            warmup_iters=cfg.SOLVER.WARMUP_ITERS,
                                            warmup_method=cfg.SOLVER.WARMUP_METHOD,
                                            patience=int(cfg.patience) if cfg.patience else 0,
